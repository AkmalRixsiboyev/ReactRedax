...
function DarajaTable({darajaUsers, DelUser, setDarajaUsers, toggleModal, editToggleModal, search}) {
    ...
    function EditModal(id) {
        editToggleModal()
        const newName = darajaUsers.filter(item=>item.id===id?item.name:'')
        setedituser(newName[0])

    }

    function saveEditModal () {
        let num = darajaUsers.map(item=>item.id===edituser.id?edituser:item)
        setDarajaUsers(num)
        editToggleModal()
    }
